"""
Fleet launch bridge -- executes and supervises fleet-coordinator's
coordinator_node as a subprocess on behalf of argOS's Plan Mission "Launch"
step (see main.py's /mission/launch* endpoints and dashboard.html's launch
panel, step 2 of Plan Mission -- step 1 is the mission draft/export).

Not wrapped as a server the way Spooky is. Spooky's job (given inputs,
compute a plan, return it) is a natural request/response fit for a
persistent HTTP service. coordinator_node's job is to own and drive a live
...
process-lifecycle problem behind another layer; not remove it. So this
mirrors run_bridges.py's existing subprocess-supervision pattern (spawn,
monitor, SIGTERM-then-SIGKILL shutdown) instead, with one addition:
`ros2 run` can spawn its own children, so killing takes the whole process
group (os.setsid at launch, os.killpg to stop), not just the immediate PID
-- a plain terminate() would orphan coordinator_node's own children.
...
class MissionProcess:
    ...
    def start(self, yaml_text: str, params: dict) -> tuple[bool, str]:
        ...
        cmd = [
            "ros2", "run", "fleet_coordinator", "coordinator_node",
            "--ros-args",
            "-p", f"mission_file:={self.mission_path}",
            "-p", f"use_sim_time:={'true' if params.get('use_sim_time') else 'false'}",
            "-p", f"initial_pose.publish:={'true' if params.get('initial_pose_publish') else 'false'}",
        ]
...
def main() -> None:
    ...
    def report_if_changed(message: str | None = None) -> None:
        ...
        try:
            session.post(f"{args.cloud_url}/mission/launch/status", json=body, timeout=3.0)
        except Exception as e:
    ...
    while True:
        try:
            ...
            if r.ok:
                ...
                if action == "launch":
                    ok, msg = mission.start(cmd.get("yaml", ""), cmd.get("params") or {})
                    log.info(msg)
